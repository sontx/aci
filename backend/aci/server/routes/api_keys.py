...
async def create_api_key(
    ...
    existing_api_key = crud.api_keys.get_api_key_by_name(context.db_session, context.project.id, body.name)
    if existing_api_key:
    ...
    api_key, api_key_str = crud.api_keys.create_api_key(context.db_session, context.project.id, body.name)
...
async def get_api_keys(
    ...
    # Get the API keys
    api_keys = crud.api_keys.get_api_keys_by_project(
        context.db_session,
        context.project.id,
        limit=limit,
        offset=offset
    ...
    total_count = crud.api_keys.count_api_keys_by_project(context.db_session, context.project.id)
...
async def get_api_key_by_name(
    ...
    api_key = crud.api_keys.get_api_key_by_name(context.db_session, context.project.id, api_key_name)
    if not api_key:
...
async def update_api_key(
    ...
    updated_api_key = crud.api_keys.update_api_key(context.db_session, api_key, body)
    context.db_session.commit()

    logger.info(f"Updated API key, api_key_name={api_key_name}, user_id={context.user.user_id}")
...
async def delete_api_key(
    ...
    crud.api_keys.delete_api_key_by_name(context.db_session, context.project.id, api_key_name)
    context.db_session.commit()
